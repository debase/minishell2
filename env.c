...
** Last update Tue Mar  5 13:21:41 2013 etienne debas
*/
...
void		env(t_list *sct, char **argv)
{
  ...
  if (i == 1)
    aff_env(sct);
  else if (i == 2 && my_strcmp2("-0", argv[1]) == 1)
    {
      sct->env_0 = 1;
      aff_env(sct);
      sct->env_0 = 0;
    }
  else if (i == 2 && my_strcmp2("-u", argv[1]) == 1)
    env_u_error();
  else if (i == 3 && my_strcmp2("-u", argv[1]) == 1)
    {
      i = found_in_env(argv[2], sct->env);
      aff_env_u(sct, i);
    }
  else if ((i >= 4 && my_strcmp2("-u", argv[1]) == 1)
	   || argv[1][0] != '-')
    env_no_such(argv, i);
  else
