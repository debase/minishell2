...
** Last update Thu Mar  7 22:03:42 2013 etienne debas
*/
...
void		replace_value(t_list *sct, int elem_env, char **argv)
{
  char		*new_elem_env;
  int		i;
  int		j;
  ...
  i = 0;
  while (argv[2][i])
    new_elem_env[j++] = argv[2][i++];
  new_elem_env[j] = 0;
  free(sct->env[elem_env]);
  sct->env[elem_env] = new_elem_env;
}

char		**add_env_value_less(t_list *sct, char **argv)
{
  int		i;
  char		**cpenv;
  ...
  cpenv[i] = my_strdup_add_elem_env(argv[1]);
  cpenv[i + 1] = NULL;
  my_free_cpenv(sct);
  sct->env = cpenv;
}

char		**add_env_with_value(t_list *sct, char **argv)
{
  int		i;
  char		**cpenv;
  ...
  while (sct->env[i])
    i++;
  if ((cpenv = malloc((i + 3) * sizeof (char *))) == NULL)
    return (NULL);
  i = 0;
  while (sct->env[i] != NULL)
    {
